...
from src.registry.manager import load_registry, save_registry
...
# ── Seuil IF par défaut (surchargé ensuite si fichier debug existe) ───────────
threshold_if = -0.10
if iso is not None:
    try:
...
# ── Préparer les listes pour les filtres sidebar ─────────────────────────────
if df is not None:
...
# ── Sidebar avec réglages interactifs ───────────────────────────────────────
with st.sidebar:
    st.markdown(
    ...
    st.markdown("**Seuils & simulation**")
    threshold_if = st.slider(
        "Seuil détection IF",
        min_value=-0.30, max_value=0.10, value=threshold_if,
        step=0.005, key="if_thresh"
    )
    sim_speed = st.slider(
        "Vitesse simulation (s/jour)",
        min_value=0.01, max_value=2.0, value=0.15, step=0.01,
        key="sim_speed"
    )
    # Fenêtre fixe : on utilise la valeur de config.py, mais on l'affiche
    st.markdown(f"*Fenêtre analyse : {WINDOW_SIZE} jours*")
    win_size = WINDOW_SIZE   # constante du config

    st.divider()

    # ─── Filtres région et profil ────────────────────────────────────────────
    st.markdown("**Filtres clients**")
    region_mode = st.radio(
        "Région", ["Toutes", "Choisir"],
        key="region_mode", label_visibility="collapsed"
    )
    if region_mode == "Choisir":
        if REGIONS:
            sel_region = st.selectbox("", REGIONS, label_visibility="collapsed")
            monitored_clients = sorted(df[df["region"] == sel_region]["client_id"].unique())
        else:
            monitored_clients = []
    else:
        sel_region = None
        monitored_clients = all_clients

    # Filtre par profil
    if PROFILES:
        sel_profiles = st.multiselect("Profil", PROFILES, default=PROFILES, key="profiles_filter")
        if sel_profiles:
            valid = meta[meta["profile"].isin(sel_profiles)]["client_id"].tolist()
            monitored_clients = [c for c in monitored_clients if c in valid]
...
# ── KPI bar (lire depuis le registre courant) ─────────────────────────────────
if data_ok:
    n_clients = len(monitored_clients) if monitored_clients else df["client_id"].nunique()
...
# ── Rendu des onglets avec les paramètres dynamiques ──────────────────────────
with tab_reseau:
    vue_reseau.render(df, registry, region_injection if not region_injection.empty else None)

with tab_surv:
    # Passage de tous les paramètres de la sidebar
    surveillance.render(
        df, meta, registry,
        iso, scaler, feature_names, shap_explainer,
        lstm_model, lstm_threshold,
        threshold_if, sim_speed, win_size,
        monitored_clients, pres_mode,
    )
    # Sauvegarde du registre après modifications par la simulation
